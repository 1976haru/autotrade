...
- [ ] **#76 Futures Promotion Policy 모든 단계 PASS** — [`futures_promotion_policy.md`](futures_promotion_policy.md) §3 7단계 매트릭스에 정의된 SIMULATION → SHADOW → PAPER → MANUAL → AI_ASSIST 단계를 *순차적으로* 통과. 단계 7 (`FUTURES_AI_EXECUTION_BLOCKED`)는 **본 프로젝트가 영구 차단** — 어떤 시나리오로도 활성화 검토 대상 아님.

선물 LIVE는 **주식 MVP보다 위험도가 한 등급 높다** (레버리지 / 강제청산 / 환율 / 24시간 거래) — 본 체크리스트의 모든 항목이 PR review에서 명시적으로 추적되어야 한다. **선물 AI 자동 실행은 본 프로젝트가 영구 허용하지 않으며**, 위 9-step blocker를 모두 통과해도 선물에서 LIVE_AI_EXECUTION 으로 진행할 수 없다 ([`futures_promotion_policy.md`](futures_promotion_policy.md) §9 참조).
