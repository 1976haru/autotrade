...
## 12. universe 확장 정책 (#3-09)

본 PR 시점 default 인 **10종 sample** (`REPRESENTATIVE_SYMBOLS`) 은 *1차 기능
검증용*. 운영 확장은 **유동성 상위 50 → 100 → 300 *단계적*** 으로 진행하며,
거래정지 / 관리종목 / ETF/ETN/SPAC / 신규상장 / 데이터 결측 / 거래량/대금 부족
종목은 **모두 필터로 제외**. 자세한 universe / 필터 정책 + CLI 옵션:
...
python scripts/run_backtest_real_data.py --universe custom --symbols 005930 000660
python scripts/run_backtest_real_data.py --universe liquidity_top50 \
    --min-avg-trading-value 5000000000 --min-listed-days 180
```

`liquidity_top*` 는 본 PR 시점 *데이터 source 미주입* — 별도 옵트인 PR 후에만
사용 가능. 후보가 없으면 *억지로 만들지 않는다* (빈 list).
