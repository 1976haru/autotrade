...
| 시스템 전체 주문 rate limit 윈도우 | `global_rate_limit_window_seconds` | 60 | `GLOBAL_RATE_LIMIT_WINDOW_SECONDS` |
| 시스템 전체 주문 rate limit 임계 | `global_rate_limit_max_count` | 0 (비활성) | `GLOBAL_RATE_LIMIT_MAX_COUNT` |
| AI 주문 kill-switch | `disable_ai_orders` | false (비활성) | `DISABLE_AI_ORDERS` (emergency_stop은 모든 주문 차단, 본 toggle은 requested_by_ai만 차단) |
| 총 노출 절대값 한도 | `max_total_exposure` | 0 (비활성) | `MAX_TOTAL_EXPOSURE` (모든 보유 포지션 합 + 신규 BUY 명목) |
| 총 노출 자본 대비 한도 (%) | `max_total_exposure_pct` | 0 (비활성) | `MAX_TOTAL_EXPOSURE_PCT` (자본 대비 자동 스케일) |
| 종목별 노출 자본 대비 한도 (%) | `max_symbol_exposure_pct` | 0 (비활성) | `MAX_SYMBOL_EXPOSURE_PCT` (단일 종목, 자본 대비 자동 스케일) |
| 연속 REJECTED 시 자동 emergency_stop | `auto_stop_consecutive_rejections` | 0 (비활성) | `AUTO_STOP_CONSECUTIVE_REJECTIONS` (N건 연속 REJECTED 시 emergency_stop=True + reason 'repeated_order_failure') |
| 일일(KST) 최대 주문 횟수 | `max_orders_per_day` | 0 (비활성) | `MAX_ORDERS_PER_DAY` (decision 무관, 모든 audit row 카운트) |
