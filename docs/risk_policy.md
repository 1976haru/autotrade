...
| 항목 | 코드 필드 | 기본값 |
|---|---|---:|
| 1회 주문 최대 명목 | `max_order_notional` | 1,000,000원 |
| 일일 최대 손실 | `max_daily_loss` | 200,000원 |
| 최대 보유 종목 수 | `max_positions` | 5개 |
| 종목별 최대 노출 | `max_symbol_exposure` | 1,500,000원 |
| 실거래 허용 | `enable_live_trading` | `false` |
| AI 자동실행 허용 | `enable_ai_execution` | `false` |

값은 `app/risk/risk_manager.py::RiskPolicy` dataclass의 default. 운영자가 향후 `/api/risk/policy` write 라우트(미구현)로 조정 가능하도록 설계됨.
