...
> **138 진행**: `OrderAuditLog.strategy` 컬럼 추가 + `LiveStrategyEngine`이 자동 채움 — audit row 자체로는 'A주문이 어느 전략인지' 식별 가능. 다만 live 주문의 *PnL* 매칭(BUY/SELL pair → 실현 손익)은 별도 작업이라 Scoreboard 통합은 여전히 follow-up. 현재 audit row에 strategy 정보가 누적되고 있어 통합 PR이 추가될 때 backfill 필요 X.
