...
| `release_tag` 입력 검증 (#5-05) | Step 0 — SemVer 정규식 통과 후만 후속 step 진행, 비-SemVer 입력 즉시 fail-fast |
| artifact / release file 패턴 = `*-setup.exe` *만* (#5-05) | `actions/upload-artifact@v4` + `softprops/action-gh-release@v2` 의 `path` / `files` 가 `bundle/nsis/*-setup.exe` 만 참조. `test_repository_hygiene::test_desktop_release_workflow_artifact_only_ships_setup_exe` 로 lock |
| `if-no-files-found: error` (#5-05) | upload-artifact 가 빈 결과 허용 0건 — 빈 setup.exe 가 release 에 안 올라감 |
| Release step gated by `create_release=true` (#5-05) | softprops step 에 `if: ${{ inputs.create_release }}` 조건 명시, 정적 테스트로 lock |
...
## 8-H. Phase 3 (auto-install) 준비 상태 (#5-06)

`UpdateBanner` Phase 2 (조회 + 직접 다운로드 링크) 까지 머지 완료. Phase 3
(`downloadAndInstall()` + `relaunch()`) 활성화에 필요한 *모든 사전 준비* 가
코드 / 설정 / 문서 단에서 끝났음을 본 절에 기록한다 — 실제 활성화는
별도 옵트인 PR.

### 8-H-1. 현재 invariant (본 PR 시점)

| 항목 | 값 | lock |
|---|---|---|
| `tauri.conf.json::plugins.updater.active` | `false` | `test_repository_hygiene::test_tauri_updater_inactive_in_phase2` |
| `tauri.conf.json::plugins.updater.pubkey` | `""` (빈 문자열) | 동일 |
| `tauri.conf.json::plugins.updater.endpoints[0]` | `https://github.com/1976haru/autotrade/releases/latest/download/latest.json` (placeholder) | 동일 |
| `tauri.conf.json::bundle.createUpdaterArtifacts` | `false` | 동일 |
| `desktop-release.yml` `TAURI_SIGNING_PRIVATE_KEY` env | *주석 처리* (활성 env 0건) | `test_desktop_release_workflow_phase3_signing_inactive` |
| `TAURI_PRIVATE_KEY` GitHub Secret | (운영자가 등록할 때까지) **미등록** | repo Settings 수동 확인 |
| repo 안 실 private key 파일 | **0건** | `test_no_tauri_private_key_committed` |

위 상태에서:
- `desktop-release` workflow 가 `TAURI_PRIVATE_KEY` Secret 미등록 상태로도
  **NSIS setup.exe 빌드 성공** — `createUpdaterArtifacts=false` 라 서명 step
  자체가 활성화되지 않음.
- `UpdateBanner` 가 GitHub Release `latest` 를 조회해 변경 안내 + 다운로드
  링크 노출. 자동 *설치* 0건.

### 8-H-2. Phase 3 활성화 시 필요한 GitHub Secret

| Secret 이름 | 값 | 등록 위치 |
|---|---|---|
| `TAURI_PRIVATE_KEY` | `tauri signer generate -w` 가 만든 private key 파일 *전체 내용* (Base64 + comment 헤더 포함) | repo Settings → Secrets → Actions |
| `TAURI_KEY_PASSWORD` | (선택) private key 가 password 보호된 경우 비밀번호 | 동일 |

자세한 키 관리 정책: [`docs/desktop_update_policy.md`](desktop_update_policy.md).

### 8-H-3. Phase 3 전환 직전 체크리스트

전환 *직전* 에 모두 통과 — 누락 시 자동 업데이트 깨짐 (사용자 PC 에
손상된 latest.json 캐시 가능성).

1. `tauri signer generate` 키 쌍 생성 + 운영자 안전 백업 (1Password 등)
2. public key 를 `tauri.conf.json::plugins.updater.pubkey` 에 commit
3. private key 를 `TAURI_PRIVATE_KEY` GitHub Secret 으로 등록
4. `desktop-release.yml` Tauri build step `env:` 의 signing 줄 주석 해제
5. `tauri.conf.json::plugins.updater.active=true` +
   `tauri.conf.json::bundle.createUpdaterArtifacts=true`
6. 첫 Phase 3 release 빌드가 `latest.json` 을 GitHub Release 에 첨부 — Actions
   로그에 "Signing artifact" 등장 + release 페이지에서 `latest.json` 다운로드
   가능
7. 기존 Phase 2 사용자에게 *공지* — 한 번만 수동으로 새 Phase 3 setup.exe
   설치 (그 후부터 자동 업데이트 작동)
8. 자동 업데이트 실패 시 즉시 `active=false` 로 되돌릴 hotfix PR draft 준비

자세한 8단계 절차: [`docs/auto_update_policy.md`](auto_update_policy.md) §10.

### 8-H-4. 본 PR (#5-06) 의 범위

- `tauri.conf.json` / `desktop-release.yml` *변경 0건* (Phase 2 invariant 유지).
- `docs/auto_update_policy.md` §10 신설 — Phase 3 8단계 가이드 + 체크리스트.
- `docs/exe_oneclick_installation.md` §3-2 신설 — 사용자에게 현재 수동 다운로드
  방식 + .env 보존 invariant 안내.
- 본 §8-H 신설.
- `test_repository_hygiene.py` 신규 4개 정적 가드 — `active=false` /
  `createUpdaterArtifacts=false` / 실 private key 파일 미커밋 / workflow signing
  env 주석 처리 상태 lock.

## 8-I. 사용자 `.env` 보존 invariant (#5-07)

업데이트 / 재설치 / 앱 제거 흐름에서 사용자 KIS 모의투자 키 / 계좌번호 /
로그 / 포트 캐시가 *어떻게* 보존되는지의 *단일 진실* 은
[`docs/auto_update_policy.md`](auto_update_policy.md) §11 에 통합 정리. 본 절은
*EXE 빌드 / 설치 흐름 관점에서* 보존 메커니즘만 요약한다.

### 8-I-1. 사용자 데이터 위치 (3개 — 모두 `%APPDATA%\Autotrade\` 안)

| 파일 | 역할 | 업데이트 시 |
|---|---|---|
| `.env` | 운영자가 직접 채우는 KIS 모의투자 키 / 계좌번호 / Anthropic 키 | ✅ 보존 (installer 가 *못 건드림*) |
| `logs\backend-YYYYMMDD.log` | launcher 로그 (Secret 원문 0건) | ✅ 보존 |
| `backend-port.json` | frontend ↔ sidecar 포트 캐시 | ⚠️ 다음 실행 시 자동 갱신 (재생성 가능) |

### 8-I-2. 보존 메커니즘 (4개 layer)

1. **installer scope** — NSIS setup.exe 의 설치 대상은 `%ProgramFiles%\Agent
   Trader v1\`. `%APPDATA%` 와 *물리적으로 별개* 디렉터리라 installer 가
   사용자 secret 을 *건드릴 권한* 자체 없음.
2. **bundle exclusion** — `tauri.conf.json::bundle.resources=[]`, sidecar
   PyInstaller `--onefile` 도 `backend/.env` 미포함 (`src-tauri/binaries/README.md`
   §3 명시).
3. **workflow safety guard** — `desktop-release.yml` Step 8 (Safety guard
   빌드 후) 가 bundle 안의 `.env` / `.env.local` / 인증서 / 키 파일을 검출
   하면 **FATAL** 로 빌드 차단.
4. **launcher 코드 invariant** — `app_desktop_launcher.py` 는 `.env` 를
   *읽기만*. `unlink` / `remove` / `write_text` 어떤 함수도 `.env` 대상으로
   호출하지 않음 (정적 grep 가드: `test_env_preservation_policy.py`).

### 8-I-3. 운영자 가이드 (요약)

- 앱 제거: `%APPDATA%\Autotrade\` 는 *자동 삭제 안 됨* — Windows 정책.
- 완전 삭제: 운영자가 위 폴더를 *수동* 삭제.
- KIS 키 백업: 1Password / Bitwarden 등 암호화된 저장소만 사용. clipboard /
  사진 / OneDrive / 채팅 금지.
- 자세한 매트릭스 / 트러블슈팅: [`docs/auto_update_policy.md`](auto_update_policy.md) §11.
