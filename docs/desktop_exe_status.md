...
## 8-B. 빌드 환경 재점검 (2026-05-15) — *체크리스트 #88~#96 완료 후*

체크리스트 #88~#96 main 머지 완료 후 실제 EXE/MSI 빌드 환경을 다시 점검한
결과. 본 점검은 `scripts/build_*.ps1` 을 *임의로 시도하지 않고*, 도구 부재
상태에서 build 를 "성공"으로 위장하지 않도록 환경만 진단.

### 8-B-1. 환경 점검 결과 (2026-05-15)

| 도구 | 상태 | 비고 |
|---|---|---|
| `rustc` / `cargo` / `rustup` | ❌ NOT FOUND | **Tauri build 진행 불가** |
| `~/.cargo/bin/` | ❌ NOT FOUND | |
| `tauri` (cargo subcommand) | ❌ NOT FOUND | cargo 의존 |
| WiX (`wix` / `candle` / `light` / `heat`) | ❌ NOT FOUND | MSI 빌드 도구 부재 |
| **MSVC Build Tools** | ✅ **vswhere 로 검출** | Visual Studio Community 2022 + Build Tools 2026 + Community 2026 (workload 확인은 cargo 빌드 시점에 검증) |
| **WebView2 Runtime** | ✅ 146.0.3856.62 | Tauri 실행 시점에 필요한 런타임 — 베타테스터 PC에도 권장 |
| `node` | ✅ v24.15.0 | |
| `npm` | ✅ 11.12.1 | |
| `python` | ✅ 3.14.3 | |
| `pyinstaller` | ✅ 6.14.2 (system) | backend sidecar 빌드 가능 |
| `src-tauri/target/` | ❌ 부재 | 빌드 산출물 없음 |
| `src-tauri/binaries/` | README.md 만 | 빌드 후 sidecar 추가됨 |

### 8-B-2. 결론

| 항목 | 평가 |
|---|---|
| `scripts/build_backend_sidecar.ps1` *실행 가능 여부* | ✅ 가능 (90-A 에서 확인 — Python + PyInstaller 갖춰짐) |
| `scripts/build_windows_installer.ps1` *실행 가능 여부* | ❌ 불가 — Rust 미설치 시 `[1/5] toolchain check` 단계에서 exit 2 |
| MSI 산출 가능 여부 | ❌ 불가 — WiX 부재 + Rust 부재 |
| setup.exe (NSIS) 산출 가능 여부 | ❌ 불가 — Rust 부재 (NSIS 는 Tauri 가 자동 다운로드 가능하나 Tauri 빌드가 선행) |
| 본 머신에서 *지금* installer 생성 가능 | ❌ 불가 |
| **추가 설치 필요 항목** | Rust toolchain + WiX (또는 Tauri 가 자동 다운로드 가능) |

### 8-B-3. 다음 단계 (운영자 직접 실행 필요)

본 머신에서 EXE/MSI 를 생성하려면 운영자가 다음을 *순서대로* 설치:

#### Step 1: Rust toolchain (필수, ~300 MiB)

```powershell
winget install Rustlang.Rustup
# 또는 https://rustup.rs/ 에서 rustup-init.exe 다운로드 후 실행
# 설치 후 새 셸 열기 → rustc --version / cargo --version 으로 확인
rustup default stable-x86_64-pc-windows-msvc
```

#### Step 2: MSVC Build Tools (이미 설치된 것으로 보임 — 워크로드 확인만)

본 머신은 Visual Studio Community 2022 / Build Tools 2026 / Community 2026
이 vswhere 로 검출됨. 다음 워크로드가 포함되어 있는지 확인:
- **Desktop development with C++**
- **MSVC v143 build tools**
- **Windows 10/11 SDK**

만약 워크로드가 부족하면 Visual Studio Installer 를 열어 추가. 부재 시
신규 설치:
...
#### Step 3: Tauri CLI v2 (Rust 설치 후)

```powershell
cargo install tauri-cli --version "^2" --locked
# 새 셸에서 cargo tauri --version 으로 확인
```

#### Step 4: WiX Toolset v3 (MSI 빌드 필수)

```powershell
winget install WiXToolset.WiXToolset
# 또는 https://github.com/wixtoolset/wix3/releases 에서 v3.14 다운로드
```

> Tauri v2 가 NSIS / WiX 를 자동으로 다운로드하는 경우도 있으나, 본 빌드
> 스크립트 (`scripts/build_windows_installer.ps1`) 의 동작 검증 시점에서
> WiX 가 명시적으로 필요. Tauri 가 알아서 가져오면 별도 설치 불필요.

#### Step 5: WebView2 Runtime (베타테스터 PC 에도 필요)

본 머신은 ✅ 146.0.3856.62 설치 확인. 베타테스터에게 배포 시:
- Windows 11 은 기본 포함
- Windows 10 은 https://developer.microsoft.com/microsoft-edge/webview2/ 에서
  *Evergreen Bootstrapper* 설치 권장

#### Step 6: 빌드 실행

위 1~4 단계 완료 후 새 PowerShell 셸을 열고:

```powershell
# 1) backend sidecar (PyInstaller) — 단독 검증
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\scripts\build_backend_sidecar.ps1 -Clean

# 2) end-to-end (frontend + sidecar + Tauri MSI/NSIS)
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\scripts\build_windows_installer.ps1

# 첫 빌드는 Rust crate 다운로드 + release 컴파일로 ~10~30분 소요
# 산출물:
#   src-tauri/target/release/bundle/msi/Agent Trader v1_1.0.0_x64_en-US.msi
#   src-tauri/target/release/bundle/nsis/Agent Trader v1_1.0.0_x64-setup.exe
```

### 8-B-4. 본 점검 시점의 안전 flag 확인

| flag | 값 | 출처 |
|---|---|---|
| `KIS_IS_PAPER` | `true` | `backend/.env.example` + `backend/app/core/config.py:96` (default) |
| `ENABLE_LIVE_TRADING` | `false` | 동일 (config.py:15) |
| `ENABLE_AI_EXECUTION` | `false` | 동일 (config.py:16) |
| `ENABLE_FUTURES_LIVE_TRADING` | `false` | 동일 (config.py:17) |

본 PR 은 안전 flag default 를 *변경하지 않으며*, secret bundle 도 포함하지 *않는다*
— 빌드 자체가 진행되지 않았으므로 산출물 0개.

### 8-B-5. 결론 — 환경 부족으로 빌드 미완료

- 본 머신에서 **EXE/MSI 빌드 진행 0건** — Rust toolchain 미설치로 빌드 스크립트
  step 1 (toolchain check) 에서 차단됨.
- `backend/dist/` / `src-tauri/binaries/*.exe` / `*.msi` / `*-setup.exe` 산출물
  *0건*.
- 빌드를 *성공으로 위장하지 않음* — 본 문서에 환경 부족을 정확히 기록.
- 운영자가 Step 1~4 설치 후 Step 6 빌드 명령을 실행하면 즉시 산출 가능.
- **권장 대체 경로**: `.github/workflows/desktop-release.yml` (#86 draft) 를 정식
  활성화해 GitHub Actions Windows runner 에서 빌드 → Release artifact 자동 첨부.
  본 머신에 Rust 설치 부담 없이 베타테스터 배포 가능.
