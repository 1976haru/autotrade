...
- **Manual full-run**: 운영자가 1000건 / 10000건 검증을 원할 때는 `tests/test_stress.py`의 `LARGE_N` 상수를 직접 수정 후 실행. SQLite + StaticPool 기준 권장 시간:

| 시나리오 (155) | CI scale | manual full-run | 예상 시간 (manual) |
|---|---:|---:|---:|
| virtual stock orders | 200 | 10,000 | ~30s |
| virtual futures 200 long/short | 200 | 5,000 | ~5s |
| AI virtual proposals | 100 | 1,000 | ~5s |
| approval queue submissions | 100 | 1,000 | ~30s |
| stress duplicate client_order_id | 100 | 5,000 | ~10s |
...
| 10 | virtual stock orders audit consistency (155) | N=200 BUY/SELL 교대 → 모든 주문이 audit row 1개씩 + executed=True | ✅ 통과 |
| 11 | virtual futures orders 200건 (155) | LONG/SHORT 교대 200회 후 broker equity > 0 (slippage/fee 손실만) | ✅ 통과 |
| 12 | futures force liquidation (155) | 5x LONG 진입 → mark가 liquidation_price 도달 → 자동 강제청산 | ✅ 통과 |
| 13 | futures margin insufficient at scale (155) | 100원 잔고로 100건 주문 → 모두 insufficient_cash | ✅ 통과 |
| 14 | AI virtual proposals 100건 (155) | 모든 audit row가 requested_by_ai=True + ai_decision_meta 보존 | ✅ 통과 |
| 15 | audit log no-loss invariant (155) | 정상/REJECTED/emergency_stop 50+50+50건 → audit 정확히 150건 | ✅ 통과 |
