...
def get_performance(
    ...
    #   실패해도 봇 성과(승률/손익비/순손익)는 정상 표시(전체 실패로 번지지 않게).
    base_equity = None
    market = {"available": False, "reason": "MARKET_NOT_FETCHED"}
    try:
        from app.performance.market_index import get_market_comparison_context
        ctx = get_market_comparison_context(start=start, end=end)
        base_equity = ctx.get("base_equity_krw")
    ...
    perf = compute_performance(db, start=start, end=end, base_equity_krw=base_equity)
