...
@router.get("/system/kis-intraday-100-validation/latest")
def get_kis_intraday_100_validation_latest() -> dict:
    """KIS-INTRADAY-100-VALIDATION-01 — 실제 KIS 분봉 100종목 전략 최종 판정 (latest, read-only).

    `reports/strategy_validation/kis_intraday_100_latest.json` 가 있으면 그 요약을 반환,
    **없으면 empty fallback**. 무거운 backtest / KIS 수집은 API 에서 실행하지 않음(CLI
    `run_kis_intraday_100_validation.py` 전용). 주문 / KIS 주문 API 호출 0건, secret 0건.
    """
    import json
    from pathlib import Path

    latest = Path("reports/strategy_validation/kis_intraday_100_latest.json")
    if latest.exists():
        try:
            data = json.loads(latest.read_text(encoding="utf-8"))
            data["_source"] = "report_file"
    ...
    return {
        ...
        "one_line_conclusion": "아직 KIS 분봉 수집/검증 리포트가 없습니다.",
        "developer_verdict": "BLOCKED",
        "requested_symbols": 0,
        "collected_symbols": 0,
        "pass_count": 0,
        ...
        "no_profit_guarantee": True,
        "disclaimer": (
            "KIS 분봉 100종목 최종 판정 리포트가 아직 없습니다. CLI "
            "`run_kis_intraday_100_validation.py --write-latest` 실행 후 갱신됩니다. "
...
def get_risk_filter_validation_latest() -> dict:
    ...
    `reports/backtest/risk_filter_only_latest.json` 가 있으면 반환, 없으면 가벼운
    NOT_READY fallback. RISK_FILTER 는 런타임/전략에 자동 적용되지 않음 (연구용).
    주문 / KIS API 0건.
    """
    import json
    from pathlib import Path

    latest = Path("reports/backtest/risk_filter_only_latest.json")
    if latest.exists():
        try:
            data = json.loads(latest.read_text(encoding="utf-8"))
            data["_source"] = "report_file"
    ...
    return {
        "_source": "empty", "available": False,
        "verdict": "RISK_FILTER_REJECTED",
        "recommendation": "리포트 없음 — CLI run_risk_filter_validation.py --write-latest 실행 후 갱신.",
...
@router.get("/system/intraday-1y-scaled-validation/latest")
def get_intraday_1y_scaled_validation_latest() -> dict:
    """1년 데이터 10/25/50 확장 검증 (latest, read-only). 없으면 empty fallback."""
    import json
    from pathlib import Path

    latest = Path("reports/strategy_validation/intraday_1y_scaled_validation_latest.json")
    if latest.exists():
        try:
            data = json.loads(latest.read_text(encoding="utf-8"))
            data["_source"] = "report_file"
    ...
    return {
        "_source": "empty", "available": False,
        "final_verdict": "SCALE_FAIL",
        "locked_rule_name": "FORWARD_UNIVERSE_60D_WEEKLY_LOCKED_V1",
        "rule_hash_match": True, "no_parameter_change": True, "no_look_ahead": True,
        "trading_days": 0, "data_quality": {},
        "exe_rebuild_recommendation": "EXE 재빌드 보류 (리포트 없음)",
        "paper_rehearsal_recommendation": "Paper 리허설 아직 불가",
        "stage_10": {}, "stage_25": {}, "stage_50": {}, "scale_analysis": {},
        "monthly_quarterly": {}, "half_split": {}, "worst_month": {}, "symbol_split": {},
        "agent_compare": {}, "slippage_stress": {}, "defense_analysis": {},
        "breadth_dependency": {}, "repeated_selected": [], "conclusions": [], "next_steps": [],
        ...
        "contains_secret": False, "no_profit_guarantee": True,
        "safety_disclaimer": (
            "1년 10/25/50 확장 검증 리포트가 아직 없습니다. CLI "
            "`run_intraday_1y_scaled_validation.py --write-latest` 실행 후 갱신됩니다(데이터 수집 선행). "
...
def get_locked_60d_weekly_latest() -> dict:
    ...
    latest = Path("reports/strategy_validation/locked_60d_weekly_latest.json")
    if latest.exists():
        try:
            data = json.loads(latest.read_text(encoding="utf-8"))
            data["_source"] = "report_file"
    ...
    return {
        "_source": "empty", "available": False,
        "final_verdict": "LOCKED_RULE_FAIL",
        "locked_rule_name": "FORWARD_UNIVERSE_60D_WEEKLY_LOCKED_V1",
        "rule_locked_before_validation": True, "no_look_ahead": True,
        "exe_rebuild_recommendation": "EXE 재빌드 보류 (리포트 없음)",
        "paper_rehearsal_recommendation": "Paper 리허설 아직 불가",
        "original_6m_replay": {}, "last20_holdout": {}, "last40_holdout": {},
        "worst_month_holdout": {}, "symbol_split": {}, "slippage_stress": {},
        "compare_40d_monthly": {}, "risk_veto_recheck": {}, "defense_recheck": {},
        "repeated_selected": [], "conclusions": [], "next_steps": [],
        "do_not_auto_apply": True, "auto_apply_allowed": False, "is_live_authorization": False,
        "live_trading_recommendation": False, "broker_order_sent": False, "order_created": False,
        "exe_build_executed": False, "contains_secret": False, "no_profit_guarantee": True,
        "safety_disclaimer": (
            "60d/weekly 고정 룰 재검증 리포트가 아직 없습니다. CLI "
            "`run_locked_60d_weekly_validation.py --write-latest` 실행 후 갱신됩니다. "
            "이 결과는 연구/백테스트 결과이며 실전매매 권고가 아닙니다."
        ),
    }


@router.get("/system/forward-universe/latest")
def get_forward_universe_latest() -> dict:
    """WF-6M forward universe rebuild (latest, read-only). 없으면 empty fallback."""
    import json
    from pathlib import Path

    latest = Path("reports/strategy_validation/forward_universe_latest.json")
    if latest.exists():
        try:
            data = json.loads(latest.read_text(encoding="utf-8"))
            data["_source"] = "report_file"
    ...
    return {
        "_source": "empty", "available": False,
        "final_universe_verdict": "UNIVERSE_FAIL",
        "exe_rebuild_recommendation": "EXE 재빌드 보류 (forward universe 리포트 없음)",
        "paper_rehearsal_recommendation": "Paper 리허설 아직 불가",
        "rule_locked_before_test": True, "best_selector": {}, "selector_results": [],
        "lookback_rebalance_grid": [], "agent_combo": [], "defense_combo": [],
        "static_vs_forward": {}, "repeated_selected": [], "repeated_excluded": [],
        "missed_opportunity": [], "top_by_return": [], "most_stable": [],
        "overfit_warning": {}, "conclusions": [], "next_steps": [],
        "do_not_auto_apply": True, "auto_apply_allowed": False, "is_live_authorization": False,
        "live_trading_recommendation": False, "broker_order_sent": False, "order_created": False,
        "exe_build_executed": False, "contains_secret": False, "no_profit_guarantee": True,
        "safety_disclaimer": (
            "WF-6M forward universe 리포트가 아직 없습니다. CLI "
            "`run_forward_universe_validation.py --write-latest` 실행 후 갱신됩니다. "
            "이 결과는 연구/백테스트 결과이며 실전매매 권고가 아닙니다."
        ),
    }


@router.get("/system/forward-validation/latest")
def get_forward_validation_latest() -> dict:
    """WF-6M forward/OOS 검증 (latest, read-only). 없으면 empty fallback."""
    import json
    from pathlib import Path

    latest = Path("reports/strategy_validation/forward_validation_latest.json")
    if latest.exists():
        try:
            data = json.loads(latest.read_text(encoding="utf-8"))
            data["_source"] = "report_file"
    ...
    return {
        "_source": "empty", "available": False,
        "final_forward_verdict": "FORWARD_FAIL",
        "exe_rebuild_recommendation": "EXE 재빌드 보류 (forward 리포트 없음)",
        "paper_rehearsal_recommendation": "Paper 리허설 아직 불가",
        "rule_locked_before_validation": True, "best": {}, "candidate_scores": {},
        "monthly_forward": {}, "anchored_forward": {}, "symbol_split": {},
        "worst_month_holdout": {}, "agent_forward": {}, "conclusions": [], "next_steps": [],
        "do_not_auto_apply": True, "auto_apply_allowed": False, "is_live_authorization": False,
        "live_trading_recommendation": False, "broker_order_sent": False, "order_created": False,
        "exe_build_executed": False, "contains_secret": False, "no_profit_guarantee": True,
        "safety_disclaimer": (
            "WF-6M forward 검증 리포트가 아직 없습니다. CLI "
            "`run_wf_6m_forward_validation.py --write-latest` 실행 후 갱신됩니다. "
            "이 결과는 연구/백테스트 결과이며 실전매매 권고가 아닙니다."
        ),
    }


@router.get("/system/strategy-agent-decomposition/latest")
def get_strategy_agent_decomposition_latest() -> dict:
    """WF-6M 매매기법/Agent 분해 (latest, read-only). 없으면 empty fallback."""
    import json
    from pathlib import Path

    latest = Path("reports/strategy_validation/strategy_agent_decomposition_latest.json")
    if latest.exists():
        try:
            data = json.loads(latest.read_text(encoding="utf-8"))
            data["_source"] = "report_file"
    ...
    return {
        ...
        "exe_rebuild_recommendation": "EXE 재빌드 보류 (분석 리포트 없음)",
        "baseline": {}, "strategy_only": [], "agent_roles": [], "agent_off_comparison": {},
        "top_by_return": [], "top_low_live_risk": [], "conclusions": [],
        "do_not_auto_apply": True, "auto_apply_allowed": False, "is_live_authorization": False,
        "broker_order_sent": False, "order_created": False, "exe_build_executed": False,
        "contains_secret": False, "no_profit_guarantee": True,
        "disclaimer": (
            "WF-6M 매매기법/Agent 분해 리포트가 아직 없습니다. CLI "
            "`run_wf_6m_strategy_agent_decomposition.py --write-latest` 실행 후 갱신됩니다. "
            "이 결과는 연구/백테스트 결과이며 실전매매 권고가 아닙니다."
        ),
    }
