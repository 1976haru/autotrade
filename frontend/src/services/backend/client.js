...
export const backendApi = {
  ...
  // INTRADAY-DATA-01: 분봉 단타 전략 검증 latest (advisory, read-only).
  // 자동 적용/실전 전환/주문 0건. KIS 분봉 미구현(분봉 CSV 입력).
  intradayStrategyValidationLatest: () =>
    backendFetch("/api/system/intraday-strategy-validation/latest"),
  ...
  systemLogs: ({ source = "ALL", severity = "ALL", q = null, limit = 100 } = {}) => {
    const qs = new URLSearchParams({
      source: String(source), severity: String(severity), limit: String(limit),
    });
    if (q) qs.set("q", String(q));
    return backendFetch(`/api/system/logs?${qs.toString()}`);
  },
};
