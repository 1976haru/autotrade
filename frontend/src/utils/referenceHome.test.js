...
describe("dailyProgress — 매수 사용금액 게이지", () => {
  const _today = () => new Date().toISOString();   // V1: 오늘(KST) created_at
  it("U6: 실체결가(avg_fill_price) 기준 BUY notional 합산, 거부 제외, % 계산", () => {
    ...
    const orders = [
      { side: "BUY", quantity: 2, avg_fill_price: 500000, broker_status: "FILLED", filled_quantity: 2, created_at: _today() },
      { side: "BUY", quantity: 1, limit_price: 200000, decision: "REJECTED", created_at: _today() }, // 제외
      { side: "SELL", quantity: 1, avg_fill_price: 999999, created_at: _today() },                   // 매도 제외
    ];
  ...
  it("U6 회귀: price 필드만 있고 avg_fill_price 없으면 0 (옛 버그 재현 방지용 — 실제 row엔 price 없음)", () => {
    const orders = [{ side: "BUY", quantity: 2, price: 500000, broker_status: "FILLED", filled_quantity: 2, created_at: _today() }];
  ...
  it("V1: 과거(어제 이전) 체결은 매수 사용금액에서 제외 — 휴장일 0만이 정답", () => {
    const orders = [
      { side: "BUY", quantity: 2, avg_fill_price: 5_740_000, broker_status: "FILLED", filled_quantity: 2,
        created_at: "2026-06-05T01:00:00Z" },   // 과거(금) — 제외돼야 함
      { side: "BUY", quantity: 1, avg_fill_price: 300_000, broker_status: "FILLED", filled_quantity: 1,
        created_at: new Date().toISOString() },  // 오늘 — 포함
    ];
    const p = dailyProgress({ orders, today: { orderCount: 1 }, now: new Date() });
    expect(p.buyUsedKrw).toBe(300_000);          // 오늘분만(1148만 누적 아님)
  });
  it("한도 0이면 기본값으로 보정", () => {
