 ...
 * - 안전 배지 ("모의 전용", "KIS Paper ON", "실거래 OFF") 노출
 */
...
describe("<AutoPaperLoopCard>", () => {
  ...
  it("renders safety badges", async () => {
    ...
    expect(screen.getByTestId("badge-not-order-signal").textContent).toMatch(
      /모의 전용/
    );
    expect(screen.getByTestId("badge-paper-mode").textContent).toMatch(/KIS Paper ON/);
    expect(screen.getByTestId("badge-no-auto-apply").textContent).toMatch(
      /주문 신호 아님/
    );
  });
  ...
  it("clicking 시작 button calls autoPaperStart", async () => {
    ...
    const btn = screen.getByTestId("btn-start-auto-paper");
    fireEvent.click(btn);
  ...
  it("clicking 긴급정지 button calls autoPaperEmergencyStop", async () => {
    ...
    const btn = screen.getByTestId("btn-emergency-stop");
    fireEvent.click(btn);
    await waitFor(() =>
      expect(api.autoPaperEmergencyStop).toHaveBeenCalledTimes(1)
    );
  });
  ...
  it("no forbidden labels in card text — 'uvicorn' absent", async () => {
    const api = _mockApi();
  ...
  it("no buy/sell/place-order labeled buttons", async () => {
    ...
    for (const btn of buttons) {
      ...
      // forbidden labels — caller-facing strings that imply real orders.
      expect(text).not.toContain("place order");
      ...
      expect(text).not.toContain("실거래 시작");
      expect(text).not.toContain("실거래 활성");
  ...
  it("shows error banner when api fails", async () => {
    ...
    render(<AutoPaperLoopCard apiClient={api} pollIntervalMs={0} />);
    await waitFor(() =>
      expect(screen.getByTestId("auto-paper-error").textContent).toMatch(
        /backend unreachable/
      )
    );
  });
