...
export function AutoPaperLoopCard({
  ...
  const onStart = useCallback(async () => {
    setBusy(true);
    try {
      await apiClient.autoPaperStart();
      await refresh();
    } catch (err) {
      setError(err?.message || String(err));
    } finally {
  ...
  const onEmergencyStop = useCallback(async () => {
    setBusy(true);
    try {
      await apiClient.autoPaperEmergencyStop();
      await refresh();
    } catch (err) {
      setError(err?.message || String(err));
    } finally {
  ...
  return (
    <Card data-testid="auto-paper-loop-card">
      ...
      {/* 안내 — uvicorn 직접 실행 안내 0건 (테스트로 lock) */}
      <div
