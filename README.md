...
- 🔎 **[`docs/final_rehearsal_error_audit.md`](docs/final_rehearsal_error_audit.md)** — **최종 리허설 전 전체 오류 점검(FINAL-AUDIT-01)** — open PR / CI / backend·frontend 테스트 / security_scan / LIVE 안전 플래그 / KIS 모의 경로 / BUILD-01·02A·02B-0 / 의존성·미추적 파일 감사 + 최종 판정 (실전 승인 아님)
- 🖥️ **[`docs/final_ui_api_checklist_verification.md`](docs/final_ui_api_checklist_verification.md)** — **체크리스트 기능 UI/API 통합 검증(FINAL-UI-API-01)** — Dashboard/AISignal/Settings 19개 카드가 실제 화면에 mount + read-only GET API 연결 검증(매니페스트 19/19 PASS, frontend 59 PASS, backend 42 PASS), 주문/실전/승인 버튼·secret 부재 (실전 승인 아님)
- 🧭 **[`docs/strategy_potential_validation.md`](docs/strategy_potential_validation.md)** — **전략 가능성 종합 평가(STRATEGY-VALIDATION-01)** — 백테스트/Walk-forward/Stress/Paper 결과를 종합해 7개 sub-score + 5단계 verdict(STRONG/CAUTIOUS/RESEARCH_ONLY/NOT_READY/BLOCKED) 산출, Agent Council vs 단일 전략 우위 평가, sample fixture·Paper 부족 시 STRONG 불가(data-sufficiency 게이팅), 자동 적용/실전 전환/주문 0건 (수익 보장 아님)
- 📈 **[`docs/real_data_strategy_validation.md`](docs/real_data_strategy_validation.md)** — **실제/준실제 데이터 전략 검증(REAL-DATA-STRATEGY-01)** — sample fixture 가 아닌 실제/준실제 OHLCV(CSV/yfinance)로 매매기법+Agent 결합 전략 검증. KIS historical 시세 API 미구현(현재가만)이라 CSV/yfinance 사용, OHLCV 품질 검증(잘못된 OHLC→BLOCKED), 실데이터지만 거래<100→CAUTIOUS cap·sample fixture→RESEARCH_ONLY cap, 자동 적용/실전 전환/주문 0건 (수익 보장 아님)
- 🗂️ **[`docs/real_ohlcv_data_collection_and_validation.md`](docs/real_ohlcv_data_collection_and_validation.md)** — **실제 OHLCV 확보 + 다종목 Backtest/Walk-forward(REAL-DATA-INPUT-01)** — 10종목 깨끗한 OHLCV CSV 수집(`collect_real_ohlcv_data.py`, yfinance/existing, 품질 FAIL 미사용·수집 실패를 성공으로 표시 0건) + PASS 종목만 다종목 backtest/walk-forward 집계(`run_real_ohlcv_backtest_walkforward.py`, 종목별+aggregate, median PF/expectancy/MDD/WF, Agent vs 단일전략). 깨진 OHLC→BLOCKED·자동 보정 금지, total_trades<100·Paper 0건→실전 검토 불가, data/market/real_ohlcv gitignore, 자동 적용/실전 전환/주문 0건 (수익 보장 아님)
- ⏱️ **[`docs/intraday_ohlcv_strategy_validation.md`](docs/intraday_ohlcv_strategy_validation.md)** — **분봉 데이터 단타 전략 검증(INTRADAY-DATA-01)** — ORB/VWAP/Momentum/Gap 은 장중 단타 전략이라 *일봉(0 trade)이 아니라 분봉* 으로 검증. 분봉 CSV 품질검증(일봉이면 FAIL·OHLC 손상 FAIL·잘못된 row 드롭 WARN) + PASS 종목만 backtest/walk-forward(`run_intraday_strategy_validation.py`). caps: total_trades 0/<30→RESEARCH_ONLY·<100→CAUTIOUS·WF<40→RESEARCH_ONLY·Paper 0건→실전 검토 불가. KIS 분봉 미구현(분봉 CSV 입력), 자동 적용/실전 전환/주문 0건 (수익 보장 아님)
- 🧾 **[`docs/intraday_data_source_collection.md`](docs/intraday_data_source_collection.md)** — **실제 분봉 CSV 확보 + KIS read-only collector 준비(INTRADAY-DATA-02)** — 표준 경로 `data/market/intraday_ohlcv/` + 증권사/HTS 한글 컬럼(일자/시가/고가/저가/종가/거래량) 자동 매핑·쉼표/원 제거 normalizer(`normalize_intraday_csv.py`, 값 생성 0·드롭만) + 파일명 bar-size 추론(`005930_5m.csv`). KIS 분봉 collector 는 **안전 placeholder**(`NEEDS_OFFICIAL_ENDPOINT_CONFIRMATION` — 공식 endpoint 확인 전 실제 호출 0건, httpx/requests/order import 0건, present-only). `GET /api/system/intraday-data-source/status`. 자동 적용/실전 전환/주문 0건 (수익 보장 아님)
- 🎯 **[`docs/real_intraday_strategy_test.md`](docs/real_intraday_strategy_test.md)** — **실제 분봉 데이터 전략 가능성 최종 테스트(REAL-INTRADAY-TEST-01)** — yfinance 실제 5분봉(KOSPI 10종목·60일·종목당 4,238봉) 수집(`collect_yfinance_intraday_ohlcv.py`, .KS 변환·reason_code·합성 대체 0건) → backtest/walk-forward/Agent 비교 → **사용자 최종 판단 6단계**(PROMISING_FOR_PAPER_TEST/WORTH_MORE_RESEARCH/TOO_EARLY/STRATEGY_NEEDS_TUNING/NOT_PROMISING/BLOCKED_BY_DATA) 산출(`run_real_intraday_final_test.py`). **실측(2026-05-25): total_trades 4,719 · Agent ADDS_VALUE · walk-forward 약함 → `WORTH_MORE_RESEARCH`("연구 가치 있으나 튜닝 필요")**. 자동 적용/실전 전환/주문 0건 (수익 보장 아님)
- 🏗️ **[`docs/final_prebuild_integrated_gate.md`](docs/final_prebuild_integrated_gate.md)** — **EXE 빌드 전 통합 검증 Gate(체크리스트 11-00)** — health/config/security/DB/Universe/Portfolio/Agent/Order/Stress/Backtest/Walk-forward/Intraday/Live safety/UI·API/문서/CI 를 한 번에 점검해 **BUILD_READY / BUILD_READY_WITH_WARNINGS / BUILD_BLOCKED** 판정(`run_final_prebuild_gate.py`, `GET /api/system/final-prebuild-gate`). LIVE flag·security·broker 직접호출·KIS live 경로 등은 BLOCKED, KIS 자격 미설정/실제 분봉 WORTH_MORE_RESEARCH 등은 WARN(빌드 차단 아님). **실측(2026-05-25): BUILD_READY_WITH_WARNINGS(PASS 13/WARN 8/FAIL 0) — 빌드 가능, WARN 확인 필요**. 실전 승인 아님·주문 실행 아님·수익 보장 아님
...
- 📊 [`docs/strategy_council_backtest.md`](docs/strategy_council_backtest.md) — **4전략 + Agent Council 백테스트** — ORB/Momentum/Gap/VWAP vote + Council final_action 을 과거 OHLCV 로 검증, 승률/손익비/PF/MDD/연속손실/expectancy + regime·time_phase 버킷 + Council vs best single 비교. CLI `run_strategy_optimization.py`(JSON/MD). **Paper 성능 분석 전용 — 실전 전환·수익 보장 아님** (#6-01)
- 🔁 [`docs/strategy_council_walk_forward.md`](docs/strategy_council_walk_forward.md) — **4전략 + Agent Council Walk-forward 과최적화 방지 검증** — train/validation/test 시간 분할 + rolling, 유지율(retention)·stability_score·overfit_suspected·성과 붕괴 탐지 + regime·time_phase 안정성 + Council vs best single(OOS). CLI `run_strategy_council_walk_forward.py`(JSON/MD). **Paper 검증 전용 — 실전 전환·수익 보장 아님** (#6-02)
- 🧪 [`docs/agent_stress_test.md`](docs/agent_stress_test.md) — **Agent / Risk Gate 스트레스 테스트** — 슬리피지/부분체결/거절/미체결/stale price/급락/급등/데이터락/포트폴리오 drift/일일 손실한도/kill switch 12 악조건을 재현해 기존 안전 가드(RiskManager·손실한도·kill switch·주문품질·정합성)가 정상 동작하는지 PASS/WARN/FAIL 검증. CLI `run_agent_stress_test.py`(JSON/MD). **Paper 검증 전용 — 실제 주문 0건·실전 전환·수익 보장 아님** (#6-03)
- 📈🧠 [`docs/agent_performance_explainability_dashboard.md`](docs/agent_performance_explainability_dashboard.md) — **성과 대시보드 + AI 판단 설명** — 전략별 승률/손익비/PF/MDD/expectancy + 체결 실패율/거절률/부분체결률 + 차단 사유 TOP + Council vs best single(`PerformanceMetricsSummary`); entry_reason/counter_reason/exit_plan/risk_flags/risk_veto/sell_reason 설명 + 구버전 fallback(`AgentDecisionExplanationCard`). read-only — **주문/실전/승인 버튼 0개·수익 보장 아님** (#6-04 + #6-07)
- 🔁🎯 [`docs/post_trade_feedback_quality_score.md`](docs/post_trade_feedback_quality_score.md) — **복기 피드백 루프 + 판단 품질 고도화** — 승패 요인/과잉진입/늦은 청산/데이터 품질/장세 부적합 태그 + threshold 추천(자동 적용 안 됨·운영자 승인 필요); quality_score 를 신호 일관성/데이터 신뢰도/리스크/장세 적합도/exit_plan 으로 고도화해 **quality 낮으면 BUY→HOLD**(별도 `pre_quality_action`, RiskOfficer/exit_plan 게이트 비우회). read-only — **주문/실전 버튼 0개·실전 전환·수익 보장 아님** (#6-05 + #6-06)
...
- 🔍🤖 [`docs/kis_paper_ai_autotrade_audit.md`](docs/kis_paper_ai_autotrade_audit.md) — **KIS 모의 AI 자동매매 전체 코드 감사 (BUILD-02B-0)** — AI 판단→KIS Paper 주문 결정→fake 주문 결과→order_quality→portfolio→outcome/review/feedback 18섹션 + 권한 게이트(모드/LIVE off/자격/confidence/quality/exit_plan/notional/daily) + SELL 보유 청산만 + Live safety 를 offline/fake 로 감사. paper_autotrade_ready / ready_for_market_open_rehearsal. CLI `run_kis_paper_autotrade_audit.py` + endpoint `/api/system/kis-paper-autotrade-audit`. **실제 KIS API 0건·주문 0건(fake)·실전 승인 아님** (BUILD-02B-0)
- 🟢✅ [`docs/premarket_readiness_gate.md`](docs/premarket_readiness_gate.md) — **장 열리기 전 사전 검증 (BUILD-02A)** — 환경변수/KIS 자격 present/Paper·Live 분리/Universe/Portfolio/Agent 카드/BUILD-01 정합성/preflight/문서·Runbook/리포트 스크립트 + (full mode) ruff·pytest·security_scan·npm lint·test·build 를 한 리포트로 점검. premarket_ready / build_ready_for_offline / ready_for_market_open_rehearsal 산출. CLI `run_premarket_readiness_gate.py`(fast/full) + endpoint `/api/system/premarket-readiness`. **실제 KIS API 0건·실전 승인 아님·수익 보장 아님** (BUILD-02A)
- 🧩✅ [`docs/prebuild_program_integrity_gate.md`](docs/prebuild_program_integrity_gate.md) — **최종 빌드 전 전체 프로그램 정합성 점검 (BUILD-01)** — Universe→KIS readiness→4전략 vote→Agent Council→RiskOfficer→exit_plan→quality→결정→KIS Paper decision→fake 주문→order_quality→portfolio→outcome/review→feedback→UI/API→Live safety 16섹션을 offline/fake 로 점검 + build_ready 산출. CLI `run_program_integrity_gate.py` + endpoint `/api/system/program-integrity`. **실전 승인 아님·실제 주문 0건(결과 fake)·수익 보장 아님** (BUILD-01)
- 🛑🔒 [`docs/live_trading_off_policy.md`](docs/live_trading_off_policy.md) — **실매매 기본 OFF + KIS Paper/Live 분리 + Live Capital Review** — ENABLE_LIVE/AI/FUTURES=false·KIS_IS_PAPER=true default 고정, KIS Paper/Live host·TR·account 완전 분리(KIS_IS_PAPER=false여도 explicit gate 없이 BLOCKED·fallback 금지), Live Capital Review(operator approval+max_notional+daily limit+whitelist, READY여도 주문 0건). read-only — **실전 OFF·주문/승인 버튼 0개** (#9-01+9-02+9-03)
...
[`docs/paper_candidate_wire.md`](docs/paper_candidate_wire.md) (PaperCandidateWire) /
...
[`docs/paper_start_explanation.md`](docs/paper_start_explanation.md) (4-05) /
[`docs/paper_decision_bridge.md`](docs/paper_decision_bridge.md) (4-07) /
[`docs/paper_position_sizing.md`](docs/paper_position_sizing.md) (4-08) /
[`docs/risk_veto_priority.md`](docs/risk_veto_priority.md) (4-09) /
[`docs/paper_decision_log.md`](docs/paper_decision_log.md) (4-10) /
